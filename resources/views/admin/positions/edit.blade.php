...
<div class="page-header w-100">
    <div>
        <h3 class="page-title mb-1">Edit Position</h3>
        <p class="text-muted mb-0">{{ $position->title }}</p>
    </div>
            ...
            <form method="POST" action="{{ route('admin.positions.update', $position->id) }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="bps" class="form-control" value="{{ old('bps',$position->bps) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold">Department</label>
                        <input type="text" name="department" class="form-control" value="{{ old('department',$position->department) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold">Number of Vacancies</label>
                        <input type="number" name="vacancies" class="form-control" value="{{ old('vacancies',$position->vacancies) }}" min="0">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold">Age Limit</label>
                        <input type="text" name="age_limit" class="form-control" value="{{ old('age_limit',$position->age_limit) }}">
                    </div>
                    <div class="col-md-8">
                        <label class="form-label fw-bold">Required Qualification</label>
                        <input type="text" name="qualification_required" class="form-control" value="{{ old('qualification_required',$position->qualification_required) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold">Domicile</label>
                        <input type="text" name="domicile" class="form-control" value="{{ old('domicile',$position->domicile) }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold">Fee Amount (Rs.) <span class="text-danger">*</span></label>
                        <input type="number" name="fee_amount" class="form-control @error('fee_amount') is-invalid @enderror" value="{{ old('fee_amount',$position->fee_amount) }}" min="0" required>
                        @error('fee_amount')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3 d-flex align-items-end">
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" name="is_active" value="1" id="isActive" {{ old('is_active',$position->is_active) ? 'checked' : '' }}>
